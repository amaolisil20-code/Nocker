...
export default function Cards() {
  ...
  const remove = (id: string) =>
    Alert.alert('Excluir', 'Excluir cartão?', [
      { text: 'Cancelar' },
      { text: 'Excluir', style: 'destructive', onPress: async () => { await api.deleteCard(id); await load(); } },
    ]);
          ...
          return (
            <TouchableOpacity key={card.id} onLongPress={() => remove(card.id)} activeOpacity={0.85}>
              <View style={[s.cardBox, { borderColor: `${card.color}44` }]}>
                <LinearGradient
                  colors={[`${card.color}55`, `${card.color}11`, '#0A0A0A']}
      ...
      <Modal visible={modal} transparent animationType="slide" onRequestClose={() => setModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.modalRoot}>
          ...
          <ScrollView style={s.sheet} contentContainerStyle={{ paddingBottom: 24 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput testID="card-name" placeholder="Ex: Nubank" placeholderTextColor={theme.colors.textTertiary}
              value={name} onChangeText={setName} style={s.input} />

            <Text style={s.label}>Últimos 4 dígitos</Text>
            <TextInput testID="card-digits" placeholder="1234" placeholderTextColor={theme.colors.textTertiary}
              value={digits} onChangeText={t => setDigits(t.replace(/\D/g, '').slice(0, 4))} keyboardType="number-pad" style={s.input} />
            ...
            <TextInput testID="card-limit" placeholder="0,00" placeholderTextColor={theme.colors.textTertiary}
            ...
            <TouchableOpacity testID="save-card" style={s.saveBtn} onPress={save} disabled={saving}>
              <Text style={s.saveTxt}>{saving ? 'Salvando...' : 'Salvar cartão'}</Text>
            </TouchableOpacity>
          </ScrollView>
...
const s = StyleSheet.create({
  ...
  cardBox: { borderRadius: 24, padding: 22, marginVertical: 10, borderWidth: 1, overflow: 'hidden',
    backgroundColor: theme.colors.surface, minHeight: 210 },
  ...
  chipBtn: { paddingHorizontal: 14, paddingVertical: 9, borderRadius: 999, borderWidth: 1, borderColor: theme.colors.border, backgroundColor: theme.colors.surfaceElevated },
  chipActive: { backgroundColor: theme.colors.primary, borderColor: theme.colors.primary },
  chipTxt: { color: theme.colors.textSecondary, fontSize: 13, fontWeight: '600' },
